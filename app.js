...
// --- TEMPORARY TOOL: COPY PERCENTAGES (DAY 1 -> ALL DAYS) ---

window.copyDay1Percentages = async function() {
    if (!auth.currentUser) { alert("Please log in."); return; }
    
    const userId = auth.currentUser.uid;
    const appId = "study-plan17";
    const monthId = "2025-11"; // Target Month
    const weekId = "week4";    // Target Week

    if (!confirm(`WARNING: This will overwrite the percentage values for Days 2-7 in ${weekId} with the values from Day 1.\n\nProceed?`)) return;

    const btn = document.getElementById('copy-perc-btn');
    btn.innerHTML = "Copying...";
    btn.disabled = true;

    try {
        // 1. Fetch the Week Document
        const weekDocRef = doc(db, `artifacts/${appId}/users/${userId}/studyPlans/${monthId}/weeks/${weekId}`);
        const weekDocSnap = await getDoc(weekDocRef);
        ...
        if (days.length === 0) {
            alert("No days found in this week.");
            return;
        }

        // 2. Get Source Data (Day 1 -> Index 0)
        const sourceRows = days[0].rows; 
        if (!sourceRows) {
            alert("Day 1 has no rows.");
            return;
        }

        // 3. Loop through Target Days (Days 2-7 -> Indexes 1-6)
        let modifications = 0;
        
        for (let i = 1; i < days.length; i++) {
            const targetDay = days[i];
            
            // We assume rows align by index (Row 1 matches Row 1)
            // If target has fewer rows, we stop at the end of target.
            // If target has more rows, extra rows get no value.
            targetDay.rows.forEach((targetRow, rowIndex) => {
                if (sourceRows[rowIndex]) {
                    const sourceVal = sourceRows[rowIndex].completionPercentage;
                    
                    // Only copy if source has a value
                    if (sourceVal !== null && sourceVal !== undefined) {
                        targetRow.completionPercentage = sourceVal;
                        modifications++;
                    }
                }
            });
        ...
        if (modifications > 0) {
            ...
            alert(`Success! Updated ${modifications} rows.\n\nPlease REFRESH the page.`);
            location.reload();
        } else {
            alert("No matching rows found to update.");
        }

    } catch (e) {
        console.error("Copy failed:", e);
        alert("Error running copy script.");
    } finally {
        btn.innerHTML = "✅ Done";
    }
};

// Add the Copy Button (Temporary)
const copyBtn = document.createElement('button');
copyBtn.id = 'copy-perc-btn';
copyBtn.innerText = "📋 COPY DAY 1 %";
copyBtn.style.cssText = "position: fixed; bottom: 130px; left: 20px; z-index: 99999; background: #8b5cf6; color: white; padding: 12px 20px; font-weight: bold; border-radius: 8px; cursor: pointer; border: 2px solid white; box-shadow: 0 4px 6px rgba(0,0,0,0.3);";
copyBtn.onclick = window.copyDay1Percentages;
